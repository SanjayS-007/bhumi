...
- **`task models` output** (unset / default, first pass): all three capabilities resolved to `azure / gpt-5.2-chat`, reason `default: auto-cascade`. **The user correctly challenged this as still broken** — that's the opposite of what Addon 5 asked for, and specifically fails on the actual target (personal laptop, no Azure). Asked directly which mechanism was responsible; the honest answer was neither of the two candidates offered (no `backends.yaml` was ever built; `BHUMI_MODEL_BACKEND` was unset) — it was the single `_BACKENDS` list being reused for both explicit lookup and the "auto" default, with Azure first in that list. Fixed by splitting it into `_BACKENDS` (explicit lookup, all three) and a separate `AUTO_CASCADE = ("groq", "gemini")` (the actual default, Azure excluded). Re-verified live: default now resolves to `gemini` (Groq had no key at that point), never `azure`, unless explicitly forced.
- **Groq, tested for real, exactly per the standard demanded**: the user provided real free-tier Groq keys mid-session. Implemented round-robin across multiple keys (`GROQ_API_KEYS`, comma-separated) per their explicit follow-up ask. Attempted a real call — and hit a real, verifiable network block: this org's Zscaler proxy denies `api.groq.com` outright (`"Not allowed to browse Generative AI and ML Applications category"`), confirmed a second, independent way via the actual SDK call (`openai.APIConnectionError`). This is a network policy finding, not a key or code problem — same evidentiary standard as the Gemini `curl` test. **A second real bug was caught in the process**: the existing live Groq test went through `select.py`'s fallback wrapper, which silently swallowed that exact connection error and substituted the deterministic backend's output — a false pass, exactly the failure mode the user was pressing on. Fixed by calling the concrete Groq backend class directly (bypassing the fallback) and asserting Groq-specific output; the fixed test now correctly fails with the real error, then reclassified as an explicit `pytest.skip` only for that specific, diagnosed exception (any other failure still fails for real).
...
- **Final state, verified**: `task models` never defaults to `azure` on any machine now; Groq round-robin is real, unit-verified code, confirmed unreachable (not confirmed working) from this specific network; Gemini remains confirmed broken by Google. 87 passed, 2 skipped (GPU-gated Tier 3; Groq's now-honest network-block skip), `ruff check .` clean.
