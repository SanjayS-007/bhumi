...
def multi_hop(session: Session, start: str, rels: list[str], max_hops: int = 5) -> list[list[str]]:
    """BFS over the adjacency table, following only the given relation
    types, in either direction. Returns every simple path found."""
    edges = session.execute(select(GraphEdge).where(GraphEdge.rel.in_(rels))).scalars().all()
